...
class BreakoutNetwork:
    def __init__(self, frame_size, resize_factor, n_actions, loss_function, optimiser):
        ...
        self.initialiser = tf.keras.initializers.RandomUniform(minval=-0.0002, maxval=0.0005,
                                                               seed=None)  # a weights initialiser: this prevents high initial Q-values
        ...
        for i, entry in enumerate(self.network_params):
            model.add(layers.Conv2D(entry[0], entry[1], strides=entry[2], activation='linear',
                                    input_shape=self.reduced_frame_size,
                                    kernel_initializer=self.initialiser))  # convolutional layer
        ...
        model.add(layers.Dense(self.network_params[-1][0],
                               activation='relu',
                               kernel_initializer=self.initialiser))  # first dense layer: connect to last conv. layer
        model.add(layers.Dense(n_actions, activation='linear',
                               kernel_initializer=self.initialiser))  # second dense layer: output actions
...
class BreakoutDQNLearner:
    ...
    def updateNetwork(self, use_replay_buffer=True, nsamples_replay_buffer=1, train_batch_size='auto', epochs=1,
                      experiences=None):
        ...
        for i, exp in enumerate(experience_batch):
            target_matrix[i, :] = self.prediction_network.predictQVectorFromFrame(
                exp[self.buffer_indices['start_frame']])  # no loss for non-represented action
            if exp[self.buffer_indices[
                'game_over']] == False:  # not a game over state - add the max Q of the next frame to the action taken
                max_Q = np.max(self.target_network.predictQVectorFromFrame(
                    exp[self.buffer_indices['result_frame']]))  # from frame resulting from action
                target_matrix[
                    i, exp[self.buffer_indices['action']]] = self.discount_factor * max_Q  # replace by max_Q target
            target_matrix[i, exp[self.buffer_indices['action']]] += exp[
                self.buffer_indices['reward']]  # update with reward associated with that action in that sample
    ...
    def _selectEpsilonGreedy(self, Q_vector, epsilon=0.8):
        ...
        if random.random() < epsilon:  # pick highest rewarding (exploit)
            max_values = np.where(Q_vector == np.amax(Q_vector))[
                0]  # all maximum value indices; prevent determinism on first index (argmax)
    ...
    def resetAndRandomNonZeroMove(self):
        ...
        self.current_frame, _, self.game_over, _ = self.game.step(
            random.choice(range(1, self.game.action_space.n)))  # prevent getting stuck in zero-moves
        return
