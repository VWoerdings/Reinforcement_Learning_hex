...
class TerminatorHex:
    ...
    def terminator_move(self, board, do_tile_increase=True):
        ...
        Args:
            board (HexUI): The current hex board.
    ...
    def terminator_min_max(self, hex_board, depth, max_or_min):
        """Returns the best position according to the min-max algorithm. Color is determined from HexUI.blue_to_move.
        Args:
            hex_board (HexUI): The current hex board.
        ...
        Returns:
            (int, int): AI player move.
        """
        board_state = hex_board.board
        move_color = [HexBoard.RED, HexBoard.BLUE][hex_board.blue_to_move]
        opposite_color = HexUI.get_opposite_color(move_color)
        ...
        if self.do_transposition:
            ...
            moves = order_moves_TT(hex_board.board, move_color, max_or_min, self.transposition_table[depth + TT_offset])
        else:
        ...
        if depth == 0 or is_game_over or len(moves) == 0:  # leaf node or game over
            ...
            for move in moves:
                deepened_board = copy.deepcopy(board_state)
                deepened_board.set_position(move, move_color)
                ...
                new_value, alpha, beta = alpha_beta(deepened_board, opposite_color, depth - 1, 'min', alpha, beta,
                ...
                if (new_value > value):
                    ...
                    best_move = move
                    if self.do_transposition:
                        self.transposition_table[depth + TT_offset][board_as_hash_key(hex_board.board)] = value
            return best_move
            ...
            for move in moves:
                ...
                new_value, alpha, beta = alpha_beta(deepened_board, opposite_color, depth - 1, 'max', alpha, beta,
                                                    self.heuristic_evaluator, depth_weighting=self.depth_weighting,
                                                    transposition_table=self.transposition_table, TT_offset=TT_offset)
                if (new_value < value):
                    value = new_value
...
def minimax(hex_board, maximiser_color, depth, max_or_min, evaluator):
    """The minimax algorithm on the HexBoard
        ...
        """
    moves = get_free_positions(hex_board)
    is_game_over = False
    if check_win(hex_board, hex_board.BLUE) or check_win(hex_board, hex_board.RED):
        is_game_over = True
    opposite_color = HexUI.get_opposite_color(maximiser_color)
    ...
    if depth <= 0 or is_game_over or len(moves) == 0:  # end state
        ...
        for move in moves:
            ...
            deepened_board.set_position(move, maximiser_color)
            new_value = minimax(deepened_board, opposite_color, depth - 1, 'min', evaluator)
        ...
        for move in moves:
            deepened_board = copy.deepcopy(hex_board)
            deepened_board.set_position(move, maximiser_color)
            new_value = minimax(deepened_board, opposite_color, depth - 1, 'max', evaluator)
            value = [value, new_value][new_value < value]
        return value
...
def alpha_beta(hex_board, maximiser_color, depth, max_or_min, alpha, beta, evaluator, depth_weighting=0, transposition_table=None, TT_offset=0):
    """The minimax algorithm on the HexBoard with alpha-beta-pruning
        Args:
            hex_board (HexBoard): The current hex board.
            maximiser_color (int): the Hex color to maximise
            depth (int): maximum depth to search
            max_or_min (str): maximise or minimise the current color to play (hex_board.blue_to_move). Is either 'min' or 'max'.
    ...
    if depth > 0:
        if use_transposition:
            moves = order_moves_TT(hex_board, maximiser_color, max_or_min, transposition_table[depth + TT_offset])
        else:
            moves = get_free_positions(hex_board)
    else:
    ...
    if (depth <= 0 or is_game_over or len(moves) == 0):  # end state
        ...
        for move in moves:
            deepened_board = copy.deepcopy(hex_board)
            deepened_board.set_position(move, maximiser_color)
            new_value, _, _ = alpha_beta(deepened_board, opposite_color, depth - 1, 'min', alpha, beta, evaluator,
        ...
        for move in moves:
            deepened_board = copy.deepcopy(hex_board)
            deepened_board.set_position(move, maximiser_color)
            new_value, _, _ = alpha_beta(deepened_board, opposite_color, depth - 1, 'max', alpha, beta, evaluator,
                                         depth_weighting=depth_weighting, transposition_table=transposition_table,
                                         TT_offset=TT_offset)
            if new_value < value:
                value = new_value
...
def order_moves_TT(hex_board, color, max_or_min, transposition_table, return_key_values=False):
    """Used with the transposition table algorithm. If we have a transposition table bound to self,
        ...
        Args:
            ...
            color (int): A color with which to fill moves
    ...
    """
    moves = [[move, 0] for move in get_free_positions(hex_board)]  # [move, score]
...
def board_hash(hex_board, maximiser_color):
    ...
    """
    board_list = []
    for k in hex_board.keys:
        board_list.append(hex_board[k])
...
def board_dijkstra(hex_board, maximiser_color):
    ...
    for e in edge_positions:
        distances = {key: float('inf') for key in hex_board.keys}  # initiate distances
        if (hex_board[e] != opponent_color):  # continuing if this is true is useless because opponent has captured this edge tile
            unvisited = [k for k in hex_board.keys]  # prevent node revisiting
            ...
            while (len(unvisited) > 0):
                ...
                neighbours = get_neighbors(hex_board, hex_to_consider)
                for neigh in neighbours:
                    move_cost = 0  # assuming it is of the right color
                    if (hex_board[neigh] == opponent_color):
                        move_cost = float('inf')  # intraversible tile
                    elif (hex_board[neigh] == hex_board.EMPTY):
