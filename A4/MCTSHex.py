...
class MCTSHex:
    ...
    def MCTS_move(self, game, cull_tree=False, move_head=True): # retrieve a MCTS move
        """
        Perform the MCTS algorithm and return a valid move.
        Args:
            game (HexUI): the current board state
        ...
        color = [HexBoard.RED, HexBoard.BLUE][game.blue_to_move] # determine color to move
        board_state = game.board
        curr_last_move = None,
        if len(game.move_list) > 0:
            curr_last_move = game.string_to_coord(game.move_list[-1])
        ...
        if cull_tree == True: # cull the previous MCTS tree
            ...
            if self.tree_head == None or game.move_list == None or game.move_list == [] or self.previous_board == None: # tree doesn't exist or isn't useful
                self.cull_tree()
                self.tree_head = MCTSNode(curr_last_move, [HexBoard.BLUE, HexBoard.RED][color == HexBoard.BLUE])
            else:
                if self.previous_board.move_list != [game.string_to_coord(mv) for mv in game.move_list[0:len(self.previous_board.move_list)]]: # non-matching boards, cull tree
                    self.cull_tree()
                    self.tree_head = MCTSNode(curr_last_move, [HexBoard.BLUE, HexBoard.RED][color == HexBoard.BLUE])
                    ...
                    for move in board.move_list[len(self.previous_board.move_list):]: # for all new moves
                        ...
                        for child in self.tree_head.children:
                            if child.move == game.string_to_coord(move):
        ...
        for t in range(self.N_trials): # do N_trials rollouts
            copy_board = copy.deepcopy(board_state)
            ...
            if self.enh_EnsureTopLevelExplr: # force exploration of the top-level move nodes, one below the root, i.e. all valid moves
                if self.tree_head != None and self.tree_head.children != []: # not non-existent or a leaf node
                    ...
                    copy_board.set_position(node_to_explore.move, color) # update position on board
            ...
            if self.enh_EnsureTopLevelExplr and (self.tree_head != None and self.tree_head.children != []):
                path, deepened_board, check_win = self.MCTS_select(copy_board, color, self.tree_head.children[index_for_exploration], []) # path to leaf node
            else:
                path, deepened_board, check_win = self.MCTS_select(copy_board, color, self.tree_head, []) # path to leaf node
            
            if self.enh_EnsureTopLevelExplr: # add the root node to the path in case of lower level start
                path.insert(0, self.tree_head)

            rollout_start_color = [HexUI.get_opposite_color(color), color][(len(path) % 2) == 0] # color with which to start rollout
            ...
            if check_win == None: # the leaf node is not a winning situation
                ...
                deepened_board.set_position(node_to_rollout.move, HexUI.get_opposite_color(rollout_start_color)) # do the child move
                winning_color = self.MCTS_rollout(deepened_board, node_to_rollout, rollout_start_color) # perform one rollout, get the winning color
        ...
        if move_head:
            ...
            #deepened_board = self._copy_and_move(game.board, best_move.move)
            self.previous_board = copy.deepcopy(game) # update board to have matching move
            self.previous_board.set_position_auto(best_move.move)
        else:
            self.previous_board = copy.deepcopy(game) # board at current root
    ...
    def MCTS_select(self, board, move_color, node, path):
        # select child nodes until leaf reached, save node path taken
        path.append(node)
        if len(node.children) == 0: # leaf node
            check_win = get_winning_color(board) # check for a winning position
            if check_win == None: # no win --> expand
                self.MCTS_expand(board, move_color, node)
            ...
            if self.enh_FreqVisitor: # frequent visitor enhancement: upgrade number of child nodes: randomly add one extra child node
                possible_moves = get_free_positions(board)
                if len(node.children) < len(possible_moves): # node not yet fully expanded?
                    ...
                    node.addChildren(MCTSNode(move_pick, move_color))
                    
            selection = self.child_select(board, node, node.n_trials) # select the best child according to the UCT formula
            deepened_board = self._copy_and_move(board, selection.move, move_color)
            
            return self.MCTS_select(deepened_board, HexUI.get_opposite_color(move_color), selection, path) # recursive
    ...
    def MCTS_expand(self, board, move_color, node):
        ...
        if self.enh_WinScan == True: # WinScan enhancement
            for move in possible_moves: # here, we ignore the expansion fraction because we want to find all winning moves
                deepened_board = self._copy_and_move(board, move, move_color)
                check_win = get_winning_color(deepened_board)
                if check_win == move_color: # found a win, clear all child nodes, set winning node as only child
                    node.children = []
                    node.addChildren(MCTSNode(move, move_color))
        ...
        if self.enh_WinScan and node.children != []: # indicating that WinScan delivered a winning move
            ...
            for move in to_expand:
                new_node = MCTSNode(move, move_color) # create nodes for each move
                node.addChildren(new_node) # add move node to current selected node
        return

    def MCTS_rollout(self, board, node, start_color):
        # random (?) rollout from the given node with the given board
        color = start_color
        while True: # scary
            check_win = get_winning_color(board)
            ...
            board.set_position(picked_move, color)
            color = HexUI.get_opposite_color(color) # switch colors
    ...
    @classmethod
    def _copy_and_move(cls, board, move, color):
        # copy a HexBoard and make a move (according to board.blue_to_move)
        deepened_board = copy.deepcopy(board)
        deepened_board.set_position(move, color)
