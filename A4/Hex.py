...
class HexBoard:
    """Lightweight re-implementation of the Hex board. No graphics/UI stuff here."""
    BLUE = 1
    RED = 2
    EMPTY = 3
    ...
    def __init__(self, board_size):
        ...
        self.keys = list(self.board.keys())
...
def get_free_positions(hex_board):
    """Returns free positions on the board
    Args:
        hex_board (HexBoard): a HexBoard (not a HexUI)
    Returns:
         :obj:`list` of (int,int): List of empty positions on the board."""
    positions = []
    for k in hex_board.keys:
        if hex_board.board[k] == HexBoard.EMPTY:
            positions.append(k)

    return positions

def get_neighbors(hex_board, coordinates):
    ...
    if cx + 1 < hex_board.board_size: neighbors.append((cx + 1, cy))
    if cx - 1 >= 0 and cy + 1 <= hex_board.board_size - 1: neighbors.append((cx - 1, cy + 1))
    if cx + 1 < hex_board.board_size and cy - 1 >= 0: neighbors.append((cx + 1, cy - 1))
    if cy + 1 < hex_board.board_size: neighbors.append((cx, cy + 1))
    if cy - 1 >= 0:   neighbors.append((cx, cy - 1))
    return neighbors

def border(hex_board, color, move):
    ...
    """
    (nx, ny) = move
    return (color == HexBoard.BLUE and nx == hex_board.board_size - 1) or (
            color == HexBoard.RED and ny == hex_board.board_size - 1)

def traverse(hex_board, color, move, visited):
    ...
    """
    if not (hex_board[move] == color) or (move in visited and visited[move]): return False
    if border(hex_board, color, move): return True
    visited[move] = True
    for n in get_neighbors(hex_board, move):
        if traverse(hex_board, color, n, visited): return True
    return False

def check_win(hex_board, color):
    """Checks if there is a winner on HexBoard.
    Args:
        hex_board (HexBoard): a HexBoard (not a HexUI)
    Returns:
    ...
    for i in range(hex_board.board_size):
        if color == HexBoard.BLUE:
            ...
            move = (i, 0)
        if traverse(hex_board, color, move, {}):
            return True
    return False

def get_winning_color(hex_board):
    if check_win(hex_board, HexBoard.RED):
        return HexBoard.RED
    elif check_win(hex_board, HexBoard.BLUE):
...
class HexUI:
    ...
    def get_neighbors(self, coordinates):
        return get_neighbors(self.board, coordinates)
        
    def border(self, color, move):
        return border(self.board, color, move)

    def traverse(self, color, move, visited):
        return traverse(self.board, color, move, visited)
    ...
    def check_win(self, color):
        return check_win(self.board, color)
    ...
    def hex_to_coord(coordinates):
        ...
        y = round((y_average - HexUI.Y_PADDING) / ((HexUI.CROSS_LENGTH + HexUI.SIDE_LENGTH) / 2))
        x_int = round((x_average - (HexUI.X_PADDING + y * HexUI.HEX_SIZE / 2)) / HexUI.HEX_SIZE)
        return x_int, y
    ...
    def coord_to_hex(coordinates):
        ...
        x = coordinates[0]
        y = coordinates[1]
        x_mid = HexUI.X_PADDING + y * HexUI.HEX_SIZE / 2 + x * HexUI.HEX_SIZE
        y_mid = HexUI.Y_PADDING + y * ((HexUI.CROSS_LENGTH + HexUI.SIDE_LENGTH) / 2)
        return x_mid, y_mid
    ...
    def hex_to_string(coordinates):
        ...
        Returns:
            str: Coordinates in string format
        """
        return HexUI.coord_to_string(HexUI.hex_to_coord(coordinates))
    ...
    def draw_grid(self):
        ...
        for yi in range(self.board_size):
            for i in range(self.board_size):
                xi = HexUI.X_PADDING + yi * HexUI.HEX_SIZE / 2 + i * HexUI.HEX_SIZE
                y = HexUI.Y_PADDING + yi * ((HexUI.CROSS_LENGTH + HexUI.SIDE_LENGTH) / 2)
                hexagon = RegularPolygon(6, HexUI.HEX_SIZE, xi, y)
        ...
        self.canvas.create_line(top_border, fill='red', width=HexUI.BORDER_WIDTH)
        self.canvas.create_line(bottom_border, fill='red', width=HexUI.BORDER_WIDTH)
        self.canvas.create_line(left_border, fill='blue', width=HexUI.BORDER_WIDTH)
        self.canvas.create_line(right_border, fill='blue', width=HexUI.BORDER_WIDTH)
...
def valid_command(command, board_size):
    if bool(re.search(HexUI.PATTERN, command)):
