class TerminatorHex:
    ...
    def __init__(self, board, max_depth, heuristic_evaluator):
    ...
    def terminator_move(self)
        ...
        """
        return terminator_min_max(self.max_depth, 'max')

    def terminator_min_max(self, depth, max_or_min):
        moves = self.board.get_free_positions()
        is_game_over = False
        if (hex_board.check_win(hex_board.BLUE) or hex_board.check_win(hex_board.RED)):
            is_game_over = True
        if (depth == 0 or is_game_over or len(moves) == 0):
